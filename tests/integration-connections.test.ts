...
// Connections/Secrets V1: persistence (CRUD, list/create/update/verify/
// archive/restore, id generation) moved to
// lib/server/integration-connections-repo.ts (see
// tests/integration-connections-repo.test.ts) and
// lib/api/integration-connections.ts. This file now covers only the pure,
// storage-independent helpers that stayed in lib/integration-connections.ts —
// option lists, label getters, and the two derivation functions
// (getIntegrationConfigurationStatus, summarizeIntegrationConnections).
