...
export function ClientsForm({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-3 sm:p-6">
      ...
      <form onSubmit={submit} className="relative my-auto max-h-[calc(100vh-24px)] w-full max-w-xl overflow-y-auto break-words bg-os-surface border border-os-border p-4 rounded-md sm:max-h-[calc(100vh-48px)]">
        <h3 className="mb-2 text-lg font-semibold">{title}</h3>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <label className="block min-w-0">
            <div className="text-[12px] text-os-dim">Nombre</div>
            <input value={name} onChange={(e) => setName(e.target.value)} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1" />
          </label>
          <label className="block min-w-0">
            <div className="text-[12px] text-os-dim">Sector</div>
            <input value={sector} onChange={(e) => setSector(e.target.value)} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1" />
          </label>
          <label className="block min-w-0">
            <div className="text-[12px] text-os-dim">Estado</div>
            <select value={status} onChange={(e) => setStatus(e.target.value as ClientStatus)} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1">
          ...
          <label className="block min-w-0">
            ...
            <input value={service} onChange={(e) => setService(e.target.value)} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1" />
          </label>
          <label className="block min-w-0">
            <div className="text-[12px] text-os-dim">Presupuesto mensual Meta</div>
            <input type="number" value={metaBudgetMonthly} onChange={(e) => setMetaBudgetMonthly(Number(e.target.value || 0))} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1" />
          </label>
          <label className="block min-w-0">
            <div className="text-[12px] text-os-dim">Fecha de inicio</div>
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1" />
          </label>
          <label className="block min-w-0 sm:col-span-2">
            <div className="text-[12px] text-os-dim">Responsable</div>
            <input value={owner} onChange={(e) => setOwner(e.target.value)} className="w-full min-w-0 bg-transparent border border-os-border px-2 py-1" />
          </label>
        </div>
