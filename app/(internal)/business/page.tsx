...
const control =
  'w-full min-w-0 border border-os-border bg-os-surface px-3 py-2.5 text-[13px] text-os-text outline-none transition-colors placeholder:text-os-dim focus:border-os-border-strong';
const fieldLabel = 'mb-1.5 block text-[10px] font-semibold uppercase tracking-[0.18em] text-os-dim';
...
export default function BusinessPage() {
  ...
  return (
    <div className="space-y-7 pb-8">
        ...
        right={
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={handleSave}
              disabled={saving}
              className="inline-flex items-center gap-2 border border-os-text bg-os-text px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-os-bg transition-opacity hover:opacity-80 disabled:cursor-wait disabled:opacity-50"
            >
      ...
      {feedback && (
        <div className={`border px-4 py-3 text-[12px] ${feedback.tone === 'ok' ? 'border-os-ok/40 bg-os-ok/5 text-os-ok' : 'border-os-err/40 bg-os-err/5 text-os-err'}`}>
          {feedback.text}
        </div>
      )}
      ...
      <section>
        ...
        <div className="grid border border-os-border bg-os-surface lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.55fr)]">
          <div className="space-y-5 border-b border-os-border p-5 sm:p-6 lg:border-b-0 lg:border-r">
            <label>
            ...
            <label>
              ...
              <textarea
                className={`${control} min-h-40 resize-y leading-6`}
      ...
      <section>
        ...
        <div className="grid gap-px border border-os-border bg-os-border md:grid-cols-2 xl:grid-cols-4">
          <div className="bg-os-surface p-5">
            <Target className="mb-6 h-4 w-4 text-os-dim" />
          ...
          <div className="space-y-3 bg-os-surface p-5">
      ...
      <section>
        ...
        <div className="grid gap-px border border-os-border bg-os-border lg:grid-cols-2">
          <label className="bg-os-surface p-5">
      ...
      <section>
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 border border-os-border bg-os-surface px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-os-muted hover:border-os-border-strong hover:text-os-text"
          >
        ...
        <div className="space-y-3">
          ...
          {services.map((service, index) => (
            <article key={service.id} className={`border border-os-border bg-os-surface p-5 sm:p-6 ${service.active ? '' : 'opacity-60'}`}>
              <div className="grid gap-4 lg:grid-cols-[minmax(220px,1.2fr)_minmax(180px,0.7fr)_140px_150px_auto] lg:items-end">
                <label>
                ...
                <label className="flex h-[42px] items-center gap-2 border border-os-border px-3 text-xs text-os-muted">
                  <input
                    type="checkbox"
                ...
                <button type="button" onClick={() => removeUnsavedOrDeactivate(index)} className="grid h-[42px] w-[42px] place-items-center border border-os-border text-os-dim hover:border-os-err hover:text-os-err" aria-label={service.id.startsWith('draft-') ? 'Eliminar servicio' : 'Desactivar servicio'}>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)]">
                <label>
                  ...
                  <input className={control} value={service.description ?? ''} onChange={(event) => updateService(index, { description: event.target.value || null })} placeholder="Qué incluye y qué se entrega" />
                </label>
                {service.billingType === 'one_off' ? (
                  <div>
                    ...
                    {service.allowTwoPayments && (
                      <input className={control} value={service.secondPaymentTrigger ?? ''} onChange={(event) => updateService(index, { secondPaymentTrigger: event.target.value || null })} placeholder="Cuándo se cobra el segundo pago" />
                    )}
                  </div>
                ) : (
      ...
      <section>
        ...
        <label className="block border border-os-border bg-os-surface p-5 sm:p-6">
